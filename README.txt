...
--------------------------------------------------------------------------------
The virtual environment for this project is located at
Y:\Project Management\Projects\Outpatient Geocoding\Geocode App\geocode_virtualenv.
The virtual environment is based on the virtualenv package in python and can be
loaded using 'geocode_virtualenv\Scripts\activate'.  A requirements file is
available, so recreate the same packages using the same versions run the command
'pip install -r requirements.txt' once in the virtual environment.

More about virtualenv can be found at:
